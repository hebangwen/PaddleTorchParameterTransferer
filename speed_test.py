...
if __name__ == "__main__":
    ...
    index_y = paddle.randint(low=0, high=64, shape=(batch_size, num_joint * 2))
    idx_zy = paddle.randint(low=0, high=64, shape=(batch_size, num_joint * 2, 64, 64))
    ...
    for j in range(100):
        ...
        for i in range(batch_size):
            for idx, (ix, iy) in enumerate(zip(index_x[i], index_y[i])):
                joint_y_for[i, idx, 0] = idx_zy[i, idx, iy, ix]
